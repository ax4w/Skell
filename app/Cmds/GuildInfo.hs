...
import Discord.Types
    ( Message(messageId, messageChannelId, messageGuildId, messageAuthor),
      ...
      Guild (guildName, guildBanner, guildIcon, guildChannels, guildNSFWLevel, guildDescription, guildAfkTimeout, guildEmojis, guildMembers, guildMaxMembers),
                    ...
                    R.messageDetailedEmbeds = Just
                      [ def
                        { createEmbedTitle = guildName guild',
                          ...
                          createEmbedDescription = fromMaybe "" (guildDescription guild'),
                          createEmbedThumbnail = Just
                            (CreateEmbedImageUrl $ buildGuildImgFromHash guildid' $ fromMaybe "" $ guildIcon guild'),
                          createEmbedImage = Just
                            (CreateEmbedImageUrl $ buildGuildImgFromHash guildid' $ fromMaybe "" $ guildBanner guild'),
                          createEmbedFields =
                            [
                              EmbedField "Bans"  (
                                T.intercalate "\n" $
                              ...
                              EmbedField "Channels" (
                                T.intercalate "\n" $
                                filter (`notElem` ["Text Channels", "Voice Channles"]) $ channelName <$> gchannels
                              ) (Just True),
                              EmbedField "Roles" (
                                T.intercalate "\n@" $
                                map roleName groles
                              ...
                              EmbedField "AFK-Timeout" (
                                T.pack $ show (guildAfkTimeout guild' `div` 60)  <> "Minutes"
                              ) (Just True),
                              EmbedField "NFSW-Level" (
                                T.pack $ show $ guildNSFWLevel guild'
                              ) (Just True),
                              EmbedField "Emojis" (
